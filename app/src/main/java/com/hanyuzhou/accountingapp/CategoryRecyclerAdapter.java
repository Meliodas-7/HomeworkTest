...
public class CategoryRecyclerAdapter extends RecyclerView.Adapter<CategoryViewHolder> {
    ...
    private LinkedList<CategoryBean> cellList = Icon.getInstance().costRes;
 ...
 /*   public String getSelected() {
    ...
    public CategoryRecyclerAdapter(Context context){
        Icon.getInstance().setContext(context);
    ...
    public void onBindViewHolder(CategoryViewHolder holder, int position) {
        final CategoryBean res = cellList.get(position);
    ...
    public void changeType(RecordBean.RecordType type){
        if (type == RecordBean.RecordType.RECORD_TYPE_EXPENSE){
            cellList = Icon.getInstance().costRes;
        }else {
            cellList = Icon.getInstance().earnRes;
        }
